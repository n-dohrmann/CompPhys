...
!  this is is copy of the code that uses the < check (as opposed to <=)
! 
! ***
...
! the issue is that this relies on an array to store the rands 
! try to get around this - find each rand in a loop and then determine
! if those rands are a hit or not - this also allows for int64's to be used

real , dimension(:), allocatable :: rands
real rnd, lin_max
integer(int64) trial_list(40)
integer(int64) :: n_trials
...
call execute_command_line('> real_le_data')
open(1, file="real_le_data",status="unknown",position="append")
